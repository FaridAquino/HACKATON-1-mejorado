...
public class OpenAIChatService {
    ...
    public OpenAIChatService(UsuarioRepository usuarioRepository, SolicitudRepository solicitudRepository) {
    ...
    public Solicitud chat(requestSolicitudDTO solicitudDTO, Long id) {
        ...
        for (Limites limite : limites) {
            if (limite.getModelo()== LimitesModelo.GPT && limite.getTokenSobrantes()==0){
                solicitud.setRespuesta_estado(solicitudEstado.RECHAZADA);
                solicitud.setRespuesta("No puedes acceder al modelo de chat GPT porque no tienes tokens");
                return solicitudRepository.save(solicitud);
            }

            if (limite.getModelo() == LimitesModelo.GPT){
        ...
        if (completions.getChoices() != null && !completions.getChoices().isEmpty()) {
            ...
            solicitud.setRespuesta_estado(solicitudEstado.RECHAZADA);
            solicitud.setRespuesta("Error, no se pudo responder la consulta con CHATGPT");
            return solicitudRepository.save(solicitud);
        }
    }
}